-- ============================================
-- ESSENTIALS IMPORT SYSTEM - FIXED VERSION
-- Special case: Allow new users to import essentials even on free plan
-- ============================================

-- First, let's check if the table exists and drop our attempt if needed
DROP TABLE IF EXISTS preset_list_usage CASCADE;
DROP TABLE IF EXISTS preset_list_items CASCADE;

-- Check existing structure and add missing columns if needed
DO $$
BEGIN
    -- Check if preset_lists table exists
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'preset_lists') THEN
        -- Create the table if it doesn't exist
        CREATE TABLE preset_lists (
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'preset_lists' AND column_name = 'slug') THEN
            ALTER TABLE preset_lists ADD COLUMN slug TEXT UNIQUE;
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'preset_lists' AND column_name = 'item_count') THEN
            ALTER TABLE preset_lists ADD COLUMN item_count INTEGER DEFAULT 0;
        END IF;
    END IF;
END $$;
...
-- Insert or update the essentials preset list
-- First check what list_type values are allowed
DO $$
DECLARE
    v_list_id UUID;
    v_list_type TEXT;
BEGIN
    -- Try to determine valid list_type from existing constraint or data
    BEGIN
        -- Try with 'standard' first
        INSERT INTO preset_lists (slug, name, description, item_count, list_type) 
        VALUES (
            'essentials',
    ...
    EXCEPTION WHEN check_violation THEN
        -- If 'standard' doesn't work, try 'curated'
        BEGIN
            INSERT INTO preset_lists (slug, name, description, item_count, list_type) 
            VALUES (
                'essentials',
            ...
            DO UPDATE SET 
                ...
                item_count = EXCLUDED.item_count;
        EXCEPTION WHEN check_violation THEN
            -- If that doesn't work either, try without list_type
    ...
    -- Clear existing items for this list (in case we're re-running)
    DELETE FROM preset_list_items WHERE preset_list_id = v_list_id;
...
BEGIN
    SELECT COUNT(*) INTO v_list_count FROM preset_lists WHERE slug = 'essentials';
    SELECT COUNT(*) INTO v_item_count FROM preset_list_items 
    WHERE preset_list_id = (SELECT id FROM preset_lists WHERE slug = 'essentials');
    
    RAISE NOTICE 'Setup complete! Found % essentials list and % items', v_list_count, v_item_count;
    
    IF v_item_count != 48 THEN
        RAISE WARNING 'Expected 48 items but found %', v_item_count;
    END IF;
END $$;
