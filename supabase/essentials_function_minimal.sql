-- ============================================
-- MINIMAL IMPORT FUNCTION - Only essential fields
-- ============================================
...
BEGIN
    ...
    -- Import the items - only selecting columns we know exist
    FOR v_item IN 
        SELECT 
            item_name,
            quantity,
    ...
    LOOP
        ...
        -- Insert the item with only basic fields
...
SELECT 
    'preset_list_items columns:' as table_info,
    string_agg(column_name, ', ' ORDER BY ordinal_position) as columns
FROM information_schema.columns 
WHERE table_name = 'preset_list_items'
GROUP BY table_name;

-- Also check baby_items columns to ensure we're using the right ones
SELECT 
    'baby_items columns:' as table_info,
    string_agg(column_name, ', ' ORDER BY ordinal_position) as columns
FROM information_schema.columns 
