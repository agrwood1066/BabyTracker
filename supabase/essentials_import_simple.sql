-- ============================================
-- ESSENTIALS IMPORT - SIMPLE VERSION
-- Works with existing preset_lists structure
-- ============================================

-- Step 1: Insert the essentials list using existing columns
-- The error shows list_type='standard' in position 5, so let's use that
...
VALUES (
    ...
    'essentials',
    'standard',  -- Using 'standard' as shown in the error
    48
) 
ON CONFLICT (slug) 
DO UPDATE SET 
...
-- Step 2: Get the list ID and proceed with items
DO $$
DECLARE
    v_list_id UUID;
BEGIN
    ...
    IF v_list_id IS NULL THEN
        RAISE EXCEPTION 'Could not find or create essentials list';
    END IF;
    ...
    -- Clear any existing items for this list
...
-- Step 3: Create the usage tracking table
...
-- Step 4: Create the import function (continues in next part...)
