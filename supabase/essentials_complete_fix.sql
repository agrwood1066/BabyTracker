-- ============================================
-- COMPLETE FIX - Handles missing constraint and all issues
-- ============================================

-- STEP 1: First, check if the unique constraint exists and add it if missing
DO $$
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_constraint 
        WHERE conrelid = 'preset_list_usage'::regclass 
        AND contype = 'u'
        AND conname LIKE '%family%preset%'
    ) THEN
        -- Add the unique constraint if it doesn't exist
        BEGIN
            ALTER TABLE preset_list_usage 
            ADD CONSTRAINT preset_list_usage_family_preset_unique 
            UNIQUE(family_id, preset_list_id);
            RAISE NOTICE 'Added unique constraint to preset_list_usage';
...
-- STEP 2: Create the final working function
...
BEGIN
    ...
    -- Record the import - Handle both cases (with or without unique constraint)
    ...
    IF v_existing_usage_id IS NOT NULL THEN
        ...
        UPDATE preset_list_usage
        SET 
            items_imported = items_imported + v_imported_count,
            imported_at = NOW()
...
SELECT 
    ...
    (SELECT COUNT(*) FROM preset_lists WHERE slug = 'essentials') as preset_lists,
    (SELECT COUNT(*) FROM preset_list_items WHERE preset_list_id = (SELECT id FROM preset_lists WHERE slug = 'essentials')) as items,
    EXISTS(SELECT 1 FROM pg_constraint WHERE conrelid = 'preset_list_usage'::regclass AND contype = 'u') as has_unique_constraint;

-- STEP 4: Show what we have
SELECT 'Constraints on preset_list_usage:' as info;
